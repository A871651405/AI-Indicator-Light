...
| `CMD_OFF` | `0x04` | 关闭灯光 | 无 |
| `CMD_BUZZER` | `0x05` | 设置蜂鸣器 | [音量, 时长, 启用标志] |
| `CMD_READ_PARAMS` | `0x06` | 读取下位机参数 | 无（请求）/ [音量, 时长, 启用标志]（响应） |
...
- 字节1：音量（0-100）
- 字节2：时长（秒，0-60）
- 字节3：启用标志（1=启用蜂鸣器, 0=关闭蜂鸣器）
  - 关闭时，下位机所有蜂鸣器触发（包括上电自检、绿灯短响、红灯响铃）均不发声

**示例**：设置音量为80，时长为10秒，启用蜂鸣器

**完整帧**：
```
0xAA 0x05 0x03 0x50 0x0A 0x01 0xFC
```
...
- 命令字：`0x05`
- 数据长度：`0x03`（3字节数据）
- 数据：`0x50`（音量80）、`0x0A`（时长10秒）、`0x01`（启用）
- 校验和：`0xAA ^ 0x05 ^ 0x03 ^ 0x50 ^ 0x0A ^ 0x01 = 0xFC`
...
**响应**：下位机返回完整数据帧（非单字节响应码）

**响应帧格式**：
```
| 起始位 0xAA | 命令字 0x06 | 数据长度 0x03 | 音量 | 时长 | 启用标志 | 校验和 |
```

**响应示例**（音量50，时长5秒，蜂鸣器启用）：
```
0xAA 0x06 0x03 0x32 0x05 0x01 0x8A
```
...
- 时长：`0x05`（十进制5秒）
- 启用标志：`0x01`（1=启用）
- 校验和：`0xAA ^ 0x06 ^ 0x03 ^ 0x32 ^ 0x05 ^ 0x01 = 0x8A`

**说明**：上位机连接串口后自动发送此命令，读取下位机 Flash 中保存的蜂鸣器参数，用于同步界面显示。
...
| v1.2 | 2026-07-07 | 新增 CMD_READ_PARAMS (0x06) 读取下位机参数命令 |
| v1.3 | 2026-07-09 | CMD_BUZZER 改为3字节（新增启用标志），CMD_READ_PARAMS 返回3字节；蜂鸣器关闭时上电自检不响 |
